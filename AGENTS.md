...
- Do not use elapsed time (fixed sleeps, debounce quiet windows, or timeouts)
  as evidence that a state transition or batch is complete. Use explicit
  protocol boundaries, acknowledgements, state transitions, or condition
  polling. A timeout may bound a wait; it must not decide correctness.
