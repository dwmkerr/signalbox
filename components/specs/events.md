...
The hub appends every event to `events.jsonl` in the state dir and rebuilds its state from that file on boot; `seq` continues from the highest persisted value. The optional, local-only `transcript` field is persisted when an event is sent directly to its local hub. This is intentional: the log is local-only and already contains `cwd`, which is no less sensitive. A forwarder strips `transcript` before writing its spool or sending the event upstream, for every profile. Commands are not written, so they are never rebuilt or replayed. Only `application/json` may post (blocks cross-origin form posts from hostile pages), and bodies are capped at 1 MiB.
...
Signals and a two-line breadcrumb of the exchange, never transcripts. `prompt` and `reply` are the only content-bearing fields in normal operation and both are cropped at the emitter (the diagnostic `raw` field is opt-in via `SIGNALBOX_RAW`). The redacted profile (`SIGNALBOX_PROFILE=redacted`) drops cwd, transcript, title, prompt, reply and raw, and hashes the session id - for machines where even one line must not leave. It keeps `machine`: the value is the already-retained hostname plus random hex, so it discloses nothing further while preserving stable machine identity. Every forwarder also strips `transcript` before spooling or uplink delivery, regardless of profile, so the path stays on the machine that owns it. One exception, by design: the opt-in diagnostic `raw` payload is forwarded verbatim, and an agent hook that carries its own transcript path (Claude's `transcript_path`, for instance) puts that path in `raw`. `SIGNALBOX_RAW` is never set in normal operation, and the redacted profile drops `raw` entirely.
