...
- **Exchange history is bounded.** The reducer keeps recent committed exchanges
...
Each committed exchange contains a `prompt`, its `reply`, the `ts` of the most
recent content-bearing event applied to it, `cropped`, and the `seq` that closed
it. The reducer also holds one pending exchange per session while a turn is
incomplete.
...
`error` event supplies the missing value. An `ended` event removes the pending
exchange and committed history with the session, so it can discard an
unanswered prompt before that prompt reaches the ring.

The ring holds `hub.historyLimit` exchanges per session, with a default of 1,000,
and evicts the oldest exchange first. The hub rebuilds the ring from
`events.jsonl` on boot. A forwarder builds it from the upstream downlink.
`/state` continues to carry only the latest prompt and reply for each session.

`GET /exchanges` returns the newest requested exchanges in chronological order.
The `before` cursor selects older exchanges whose `seq` is lower. A non-empty
page returns its oldest `seq` as `next_before`; an empty page returns null. Page
until the response contains no exchanges.
...
event: sync      data: {"seq":N}  replay is complete through seq N (control only)
```
...
| `GET /stream?since=N` | Server-sent events: replay every signal after seq N, emit one `sync` control frame carrying the replay high-water seq, then continue live. Heartbeat every 15s. Commands are live-only and never appear in the replay. |
...
emitter sent, `ts` belongs to the most recent content-bearing event applied to
the exchange, `cropped` marks a pair either side of which hit its cap, and `seq`
...
Hook-path emitters append each event to `${stateDir}/spool.jsonl` before attempting delivery. On boot, a state-owning hub drains that spool completely into `events.jsonl` before binding any listener. It holds the spool's append lock across the final empty check and the synchronous listener bind, so an event cannot cross from the pre-ready side to the post-ready side unnoticed. The first successful `/state` therefore contains the final folded backlog, and `/stream` subscribers never render historical offline events as a burst of live transitions. The boundary is the empty spool under lock, not a quiet period or timeout. A timeout may still bound an individual network attempt without deciding that startup replay is complete.
...
Before its local listener is bound, a starting forwarder moves the complete hook-path `spool.jsonl` backlog into its durable `forward-spool.jsonl` in FIFO order under the same readiness boundary. It starts the upstream stream privately, consumes the complete historical replay, drains the outbound spool, and waits until every accepted event has returned through the downlink. Only then does it bind the local listener. A current upstream marks the exact replay boundary with `event: sync`; an older upstream's first heartbeat is the compatibility boundary because that implementation schedules heartbeats only after queuing its complete replay. No offline event is stranded merely because the machine changed from a local hub to a forwarder while it was stopped, and the first local `/state` is already the fully reduced upstream state.
...
The uplink opens `GET <upstream>/stream?since=<lastSeq>`, using `since=0` on first connect to build the cache from the full upstream log. During startup this connection and its replay exist before the local listener, so no local subscriber can observe intermediate cache states. It sends the resolved bearer when one is configured, resumes after the last seen upstream `seq`, and reconnects after 1 second with exponential backoff capped at 15 seconds. On a reconnect (never the first connect) the forwarder also closes every downstream stream, so its own clients resync from `/state` and adopt the upstream's seq domain (see the seq-domain note above). `signal` frames are normalized, applied to the cache, added to the bounded backlog and re-broadcast. `sync` marks replay completion and never touches state. `command` frames are only re-broadcast and never touch the cache. The same resolved credential is used for event and command POSTs; a tokenless forwarder sends no Authorization header, which supports an unauthenticated loopback upstream but normally leaves the upstream to reject the request.
...
The hub deduplicates ingest by each event's unique `id`. It keeps a bounded window of recent identifiers and seeds that window from the log tail on boot, so it catches a redelivery that spans a hub restart. Last-write-wins makes duplicate events harmless for session rows. Duplicate `done` or `busy` events can corrupt exchange history, so the hub acknowledges a repeated `id` without applying, logging, or broadcasting it.

An upstream 4xx permanently drops that event so it cannot block the queue. A network error or any other upstream status keeps it for a later pass.
...
Signalbox sends raw Markdown, including line breaks, to the configured hub. In
local mode it remains on this machine. A remote configuration sends it to the
remote hub. `prompt` and `reply` carry this content up to their caps of 1,024 and
10,240 characters, and `cropped` marks an event whose text reached a cap.
