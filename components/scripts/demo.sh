...
#
# Every seeded session carries the `demo` tag. --clear still removes them by
# key, while `!demo` in the app's Additional filters hides them without removing
# them. Fire captures the pane origin when run inside tmux, so run it from your
# recording pane if you want jump to route back.
set -euo pipefail
